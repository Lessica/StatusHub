...
(function() {
  $(document).ready(function() {
    var addAutomaticMessage, automaticMessage, formatMessageTime, messagesOlderThan, today, tomorrow;
    ...
    formatMessageTime = function(localtime) {
      var tz, tzRegex;
      tzRegex = /\(([^)]+)\)/;
      tz = tzRegex.exec(new Date())[1];
      return "" + (localtime.format('HH:mm')) + " " + tz;
    };
    $('.auto-message').each(function() {
      return $(this).remove();
    });
    today = moment();
    if ($("#" + (today.format('YYYY-MM-DD'))).length === 0 && window.location.pathname === '/messages') {
      $("<div class='message_group' id='" + (today.format('YYYY-MM-DD')) + "'><h3><time datetime='" + (today.format()) + "'>" + (today.format('MMM DD, YYYY')) + "<time></h3></div>").insertAfter('.messages-header');
    }
    $('.message').each(function() {
      var container_id, localtime;
      localtime = moment($(this).find('time').attr('datetime'));
      $(this).find('time').html(formatMessageTime(localtime));
      container_id = $("#message").length > 0 && "#message" || ("#" + (localtime.format('YYYY-MM-DD')));
      return $(this).detach().appendTo(container_id);
    });
    ...
    messagesOlderThan = function(date) {
      return $.map($('.message'), function(element) {
        var elementDate;
        elementDate = moment($(element).find('time').attr('datetime'));
        if (elementDate < date) {
          return element;
        }
      });
    };
    addAutomaticMessage = function(messageContainer) {
      var messageDateTime, olderMessages, previousStatus;
      messageDateTime = moment("" + (messageContainer.attr('id')) + " 00:00", "YYYY-MM-DD HH:mm");
      olderMessages = messagesOlderThan(messageDateTime);
      if (olderMessages.length === 0) {
        previousStatus = $('#message-list').data('last-known-status') || 'good';
      } else {
        previousStatus = $(olderMessages).first().data('status');
      }
      return $("<div class='message " + previousStatus + "'><time class='time' datetime='" + (messageDateTime.toISOString()) + "'>" + (formatMessageTime(messageDateTime)) + "</time> <span class='title'>" + (automaticMessage(previousStatus)) + "</span></div>").insertAfter(messageContainer);
    };
    tomorrow = moment().add(1, 'days').format('YYYY-MM-DD');
    $(".message_group#" + tomorrow).remove();
    return $(':not(.message_group:has(.message))').each(function() {
      return addAutomaticMessage($(this));
    });
  });
